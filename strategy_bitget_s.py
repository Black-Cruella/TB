...
try:
    with open('./TBWOTP/hp_status.txt', 'r') as file:
...
try:
    with open('./TBWOTP/lp_status.txt', 'r') as file:
...
if side == "long":
    if num_TS_orders_open < 1 and num_position_open == 1:
        ...
        trailing_stop_price = entry_price * 1.0016
...
if side == "short":
    if num_TS_orders_open < 1 and num_position_open == 1:
        ...
        trailing_stop_price = entry_price * 0.9983
        rounded_price = round(trailing_stop_price, 3)
        trailingPercent = 0.5  # 1% de suivi
...
with open('./TBWOTP/hp_status.txt', 'w') as file:
    file.write(str(hp_status))

with open('./TBWOTP/lp_status.txt', 'w') as file:
